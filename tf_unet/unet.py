...
def create_conv3d_net(x, keep_prob, channels, n_class, layers=3, features_root=16, filter_size=3, pool_size=2,
                      summaries=True):
    """
    working on it...
    for 3d Unet, same padding was used to maintain the size. therefore the size will maintain the same
    
    Creates a new 3d convolutional unet for the given parametrization.

    :param x: input tensor, shape [?,nD(depth),nH,nW,channels]
    :param keep_prob: dropout probability tensordeeddddd
    :param channels: number of channels in the input image
    :param n_class: number of output labels
    :param layers: number of layers in the net
    :param features_root: number of features in the first layer
    ...
    for documentation of 3D convolution:
    ...
    ny = tf.shape(x)[3]
    nz = tf.shape(x)[1]
    x_image = tf.reshape(x, tf.stack([-1, nz, nx, ny, channels]))  # follow NDHWC format
    in_node = x_image
    batch_size = tf.shape(x_image)[0]
    ...
    # size = in_size
    # down layers (Layer 0)
    conv_3d, w1, b1 = conv3d_dropout('conv1_0', in_node, channels, features_root, keep_prob,
                                     kernel_size=[3, 3, 3], padding='SAME', strides=[1, 1, 1, 1, 1])
    conv_3d2, w2, b2 = conv3d_dropout('conv2_0', conv_3d, features_root, features_root, keep_prob,
                                      kernel_size=[3, 3, 3], padding='SAME', strides=[1, 1, 1, 1, 1])
    dw_h_convs[0] = conv_3d2

    weights.append((w1, w2))
    biases.append((b1, b2))
    convs.append((conv_3d, conv_3d2))
    pools[0] = max_pool3d(dw_h_convs[0], pool_size)
    in_node = pools[0]
    ...
    for layer in range(1, layers):
        ...
        conv_3d, w1, b1 = conv3d_dropout('down_conv1_' + str(layer), in_node, features // 2, features, keep_prob,
                                         kernel_size=[3, 3, 3], padding='SAME', strides=[1, 1, 1, 1, 1])
        conv_3d2, w2, b2 = conv3d_dropout('down_conv2_' + str(layer), conv_3d, features, features, keep_prob,
                                          kernel_size=[3, 3, 3], padding='SAME', strides=[1, 1, 1, 1, 1])
        dw_h_convs[layer] = conv_3d2
    ...
    for layer in range(layers - 2, -1, -1):
        ...
        # up-convolution
        h_deconv = tf.nn.relu(deconv3d('deconv' + str(layer), in_node, features, pool_size))
        h_deconv_concat = tf.concat([dw_h_convs[layer], h_deconv], -1)
        deconv[layer] = h_deconv_concat
        in_node = h_deconv_concat
        # conv 3*3
        conv_3d, w1, b1 = conv3d_dropout('up_conv1_' + str(layer), in_node, features, features // 2, keep_prob,
                                         kernel_size=[3, 3, 3], padding='SAME', strides=[1, 1, 1, 1, 1])
        conv_3d2, w2, b2 = conv3d_dropout('up_conv2_' + str(layer), conv_3d, features // 2, features // 2, keep_prob,
                                          kernel_size=[3, 3, 3], padding='SAME', strides=[1, 1, 1, 1, 1])
    ...
    output_map, w1, b1 = conv3d_dropout('output', in_node, features_root, n_class, 1, kernel_size=[1, 1, 1],
                                        padding='SAME',
                                        strides=[1, 1, 1, 1, 1])
    up_h_convs["out"] = output_map

    if summaries:
        '''block this image summary for now.'''
        # for i, (c1, c2) in enumerate(convs):
        #     tf.summary.image('summary_conv_%02d_01' % i, get_image_summary(c1))
        #     tf.summary.image('summary_conv_%02d_02' % i, get_image_summary(c2))
        #
...
class Unet3D(Unet):
    def __init__(self, channels=3, n_class=2, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        self.logits, self.variables = create_conv3d_net(self.x, self.keep_prob, channels, n_class, **kwargs)
        self.cost = self._get_cost(self.logits, cost, cost_kwargs)

        self.gradients_node = tf.gradients(self.cost, self.variables)

        self.cross_entropy = tf.reduce_mean(cross_entropy(tf.reshape(self.y, [-1, n_class]),
                                                          tf.reshape(pixel_wise_softmax_3d(self.logits),
                                                                     [-1, n_class])))
...
class Trainer(object):
    ...
    def output_minibatch_stats(self, sess, summary_writer, step, batch_x, batch_y):
        ...
        logging.info(
            "Iter {:}, Minibatch Loss= {:.4f}, Training Accuracy= {:.4f}, Minibatch error= {:.1f}%".format(step, loss, acc,
                                                                                                           error_rate(
                                                                                                               predictions,
                                                                                                               batch_y)))
...
def error_rate(predictions, labels):
    ...
    if len(predictions.shape) == 5:
        ...
        err = 100.0 - (100.0 * np.sum(np.argmax(predictions, 4) == np.argmax(labels, 4)) / (
            predictions.shape[0] * predictions.shape[1] * predictions.shape[2] * predictions.shape[3]))
    else:
        err = 100.0 - (100.0 * np.sum(np.argmax(predictions, 3) == np.argmax(labels, 3)) / (
